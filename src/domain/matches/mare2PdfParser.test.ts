...
import {
	...
	summarizeOwnerDivisionAndCategory,
	summarizeOwnerDivisionAndCategoryValue,
	summarizeSnapshotDivisionsAndCategories,
...
describe("Mare2 PDF parser", () => {
	it("parses score verification text into the analysis snapshot shape", () => {
		...
		expect(snapshot.competitors[0]).toEqual(
			expect.objectContaining({ divisionId: "P", categoryId: "SS" }),
		);
		expect(snapshot.competitors[1]).toEqual(
			expect.objectContaining({ divisionId: "OP", categoryId: undefined }),
		);
		expect(snapshot.competitors[2]).toEqual(
			expect.objectContaining({ divisionId: "PCO", categoryId: "S" }),
		);
		expect(snapshot.competitors[3]).toEqual(
			expect.objectContaining({ divisionId: "PCI", categoryId: undefined }),
		);
		expect(
			summarizeOwnerDivisionAndCategory(snapshot, ["GUERZONI, MASSIMO"]),
		).toBe("Div: PRODUCTION · Cat: SS");
		expect(summarizeOwnerDivisionAndCategory(snapshot, ["101"])).toBe(
			"Div: OPTICS",
		);
		expect(
			summarizeOwnerDivisionAndCategoryValue(snapshot, ["GUERZONI, MASSIMO"]),
		).toBe("PRODUCTION · SS");
		expect(
			summarizeOwnerDivisionAndCategoryValue(snapshot, ["ROSSI, MARIO"]),
		).toBe("PCC OPTIC · S");
		expect(
			summarizeOwnerDivisionAndCategoryValue(snapshot, ["VERDI, LUIGI"]),
		).toBe("PCC IRON");
		expect(summarizeSnapshotDivisionsAndCategories(snapshot)).toBe(
			"Div: OPTICS, PCC IRON, PCC OPTIC, PRODUCTION · Cat: S, SS",
		);
		...
		expect(snapshot.scores[0]).toEqual(
			expect.objectContaining({
