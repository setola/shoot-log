...
export async function parseMare2PdfSnapshot(file: File): Promise<PractiscoreMatchSnapshot> {
  const text = await extractPdfText(file);
  return parseMare2TextSnapshot(text, file.name);
}

export function parseMare2TextSnapshot(text: string, sourceFileName = 'mare2-results.pdf'): PractiscoreMatchSnapshot {
  ...
  const stageMaxPoints = new Map<string, number>();
  ...
  for (const line of lines) {
    ...
    const stageScore = currentCompetitor ? parseStageScoreLine(line, currentCompetitor.internalMemberId) : undefined;
    if (stageScore) {
      scores.push(stageScore);
      const currentMax = stageMaxPoints.get(stageScore.internalStageId) ?? 0;
      stageMaxPoints.set(stageScore.internalStageId, Math.max(currentMax, stageScore.finalScore ?? 0));
    }
  }
  ...
  const stages: PractiscoreStage[] = [...stageMaxPoints.entries()]
    .sort(([a], [b]) => Number(a) - Number(b))
    .map(([stageId, maxPoints]) => ({
      internalStageId: stageId,
      name: `Stage ${stageId}`,
      maxPoints,
      minRounds: maxPoints > 0 ? Math.round(maxPoints / 5) : undefined
    }));
...
function findMatchName(lines: string[]): string {
  const titleIndex = lines.findIndex((line) => line.trim().toLowerCase() === 'score verification by competitor');
...
function findPrintedDate(lines: string[]): string {
  const printedLine = lines.find((line) => /Printed\s+\d{1,2}\s+\p{L}{3}\s+\d{4}/iu.test(line));
  const match = printedLine?.match(/Printed\s+(\d{1,2})\s+(\p{L}{3})\s+(\d{4})/iu);
...
function parseStageScoreLine(line: string, internalMemberId: string): PractiscoreStageScore | undefined {
  const match = line.match(/^\s*Stage\s+(\d+)\s+([\d,.]+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)(?:\s+\S+)?\s+([\d,.]+)\s*$/i);
  if (!match) return undefined;

  const [, stageId, factor, points, alpha, charlie, delta, , misses, noShoot, procedurals, time] = match;
