...
import { Plus, Save, Shield, Target, X } from "lucide-react";
import {
	EntityAddButton,
	EntityPage,
	RecordActionButtons,
...
const firearmTypes: FirearmType[] = [
	"pistol",
	"revolver",
	"rifle",
	"shotgun",
...
export function FirearmsCrud() {
	...
	const [formValues, setFormValues] = useState<FirearmFormValues>(
		createEmptyFirearmForm,
	);
	...
	return (
		<EntityPage
			title={t("firearms.title")}
			titleId="firearms-title"
			description={t("firearms.description")}
			actions={
				!showForm ? (
					...
					>
						...
						>
							<div className="form-title-row">
								<h3>
									{editingFirearm
										? t("firearms.editTitle")
										: t("firearms.createTitle")}
								</h3>
								<button
								...
								<label>
									...
									<select
										...
										onChange={(event) =>
											setFormValues({
												...formValues,
												type: event.target.value as FirearmType,
											})
										}
									>
								...
								<label>
									...
									<input
										...
										onChange={(event) =>
											setFormValues({
												...formValues,
												initialRoundCount: event.target.valueAsNumber,
											})
										}
									/>
								</label>
										...
										<label>
											...
											<input
												...
												onChange={(event) =>
													setFormValues({
														...formValues,
														acquisitionReference: event.target.value,
													})
												}
											/>
										</label>
						...
						{firearms?.map((firearm) => (
							...
							>
								...
								<div className="record-content">
									...
									<p>
										{[firearm.manufacturer, firearm.model, firearm.caliber]
											.filter(Boolean)
											.join(" · ") || t(`firearms.types.${firearm.type}`)}
									</p>
									<p className="muted">
										{t("firearms.roundCount", {
											count: firearm.initialRoundCount,
										})}
									</p>
								...
								<RecordActionButtons
									...
									onEdit={() => startEdit(firearm)}
									onDelete={() => setDeleteTarget(firearm)}
								/>
							</article>
						))}
					</div>
			...
			{deleteTarget ? (
				...
				>
					<div
						...
						role="dialog"
						aria-modal="true"
						aria-labelledby="delete-firearm-title"
						onMouseDown={(event) => event.stopPropagation()}
					>
						...
						<p>
							{t("firearms.deleteConfirm", { name: deleteTarget.nickname })}
						</p>
						<div className="dialog-actions">
							<button
								className="button button-secondary"
