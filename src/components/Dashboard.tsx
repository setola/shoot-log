import { Cloud, Package, Shield, Target, Timer, Trophy, Wrench } from 'lucide-react';
...
export function Dashboard({ onNavigate }: DashboardProps) {
  ...
  const ammoTransactions = useLiveQuery(() => db.ammoTransactions.toArray(), []);
  const maintenanceEvents = useLiveQuery(() => db.maintenanceEvents.toArray(), []);
  ...
  const currentMonthRounds = currentMonthSessions?.reduce((sum, session) => sum + session.roundsFired, 0) ?? 0;
    ...
    <div className="dashboard-stack">
      ...
      <section aria-labelledby="overview-title">
        ...
        <div className="stat-grid">
          <StatCard icon={<Target size={18} />} label={t('dashboard.stats.firearms')} value={firearms?.length ?? 0} sub={t('dashboard.stats.activeFirearms', { count: firearms?.filter((firearm) => !firearm.archived).length ?? 0 })} onClick={() => onNavigate('firearms')} accent />
          <StatCard icon={<Timer size={18} />} label={t('dashboard.stats.sessionsThisMonth')} value={currentMonthSessions?.length ?? 0} sub={t('dashboard.stats.roundsFired', { count: currentMonthRounds })} onClick={() => onNavigate('training')} />
...
function StatCard({ icon, label, value, sub, accent, onClick }: StatCardProps) {
  return (
    <button className={accent ? 'stat-card stat-card-accent' : 'stat-card'} type="button" onClick={onClick}>
